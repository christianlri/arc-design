...
WITH date_params AS (
  SELECT
    DATE('2025-12-01') AS param_month,
    DATE_TRUNC(DATE('2025-12-01'), MONTH) AS current_month,
    DATE_SUB(DATE_TRUNC(DATE('2025-12-01'), MONTH), INTERVAL 1 MONTH) AS prev_month,
    DATE_TRUNC(DATE_SUB(DATE('2025-12-01'), INTERVAL 1 YEAR), MONTH) AS prev_year_month_start,
    DATE_TRUNC(DATE('2025-12-01'), QUARTER) AS current_quarter_start,
    DATE_SUB(DATE_TRUNC(DATE('2025-12-01'), QUARTER), INTERVAL 1 QUARTER) AS prev_quarter_start,
    DATE_TRUNC(DATE_SUB(DATE('2025-12-01'), INTERVAL 1 YEAR), QUARTER) AS prev_year_quarter_start,
    DATE_SUB(DATE_ADD(LAST_DAY(DATE('2025-12-01'), QUARTER), INTERVAL 1 DAY), INTERVAL 1 DAY) AS current_quarter_end,
    DATE_TRUNC(DATE('2025-12-01'), YEAR) AS current_year_start,
    DATE_TRUNC(DATE_SUB(DATE('2025-12-01'), INTERVAL 1 YEAR), YEAR) AS prev_year_year_start,
    DATE_SUB(DATE_ADD(LAST_DAY(DATE('2025-12-01'), YEAR), INTERVAL 1 DAY), INTERVAL 1 DAY) AS current_year_end,
