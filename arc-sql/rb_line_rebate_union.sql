...
WITH rb_valid_standard_terms AS (
  SELECT * FROM `{{ params.project_id }}.{{ params.dataset.cl }}.rb_valid_standard_terms_retro`
    UNION ALL
  SELECT * FROM `{{ params.project_id }}.{{ params.dataset.cl }}.rb_valid_standard_terms`
),
valid_standard_terms_sup_amount_sku AS (
  ...
  FROM rb_valid_standard_terms AS vs
  WHERE TRUE
...
valid_standard_terms_sup_term_amount_sku AS(
  SELECT vst.global_entity_id,
    ...
    FROM rb_valid_standard_terms AS vst
    WHERE TRUE
...
final_standard_terms AS(
  SELECT va.*
    ...
    FROM rb_valid_standard_terms AS va
