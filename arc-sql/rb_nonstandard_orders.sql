...
WITH orders_current_month AS (
  SELECT o.order_id,
    ...
    -- Subtraction logic aligned with the same "treat partially_inbounded as done" rule
    CASE
      WHEN o.purchase_order_status = 'partially_inbounded'
           AND REGEXP_CONTAINS(o.country_code, {{ params.param_inbounded_country_code }})
      THEN 'done'
      ELSE o.purchase_order_status
  ...
  WHERE TRUE
    ...
    AND (o.purchase_order_status IN ('done', 'confirmed')
      OR (o.purchase_order_status = 'partially_inbounded'
           AND REGEXP_CONTAINS(o.country_code, {{ params.param_inbounded_country_code }})))
...
orders_parent_prev_month AS (
  SELECT op.order_id,
    ...
    -- Subtraction logic aligned with the same "treat partially_inbounded as done" rule
    CASE
      WHEN op.purchase_order_status = 'partially_inbounded'
           AND REGEXP_CONTAINS(op.country_code, {{ params.param_inbounded_country_code }})
      THEN 'done'
      ELSE op.purchase_order_status
    END as purchase_order_status,
    o.created_date,
    ...
    JOIN`{{ params.project_id }}.{{ params.dataset.cl }}.rb_purchase_orders` AS op 
      ON o.parent_purchase_order_id = op.order_id
        AND (op.purchase_order_status IN ('done', 'confirmed')
          OR (op.purchase_order_status = 'partially_inbounded'
              AND REGEXP_CONTAINS(op.country_code, {{ params.param_inbounded_country_code }})))
    WHERE TRUE
    /*order status 3 = done; 1 = confirmed*/
