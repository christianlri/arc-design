...
WITH valid_standard_terms_sup_amount_sku AS (
  ...
  FROM `{{ params.project_id }}.{{ params.dataset.cl }}.rb_valid_standard_terms_retro` AS vs
  WHERE TRUE
...
valid_standard_terms_sup_term_amount_sku AS(
  SELECT vst.global_entity_id,
    ...
    FROM `{{ params.project_id }}.{{ params.dataset.cl }}.rb_valid_standard_terms_retro` AS vst
    WHERE TRUE
...
final_standard_terms AS(
  SELECT va.*
    ...
    FROM `{{ params.project_id }}.{{ params.dataset.cl }}.rb_valid_standard_terms_retro` AS va
