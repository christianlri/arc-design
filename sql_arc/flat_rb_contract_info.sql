...
-- NOTE: DECLARE replaced with hardcoded values for debug run (entities: HF_EG|PY_CL|FP_SG|FP_TH|NP_HU|GV_ES|TB_JO|TB_KW|TB_OM|PY_AR|PY_PE|TB_AE|TB_QA|YS_TR|GV_UA|GV_IT|TB_BH|FP_HK|FP_PH|HS_SA, month: 2025-12-01)
-- param_month = DATE('2025-12-01')
-- param_global_entity_id = '"HF_EG|PY_CL|FP_SG|FP_TH|NP_HU|GV_ES|TB_JO|TB_KW|TB_OM|PY_AR|PY_PE|TB_AE|TB_QA|YS_TR|GV_UA|GV_IT|TB_BH|FP_HK|FP_PH|HS_SA"'
-- project_id = 'fulfillment-dwh-production'
-- dataset.curated_data_shared_salesforce_srm = 'curated_data_shared_salesforce_srm'

CREATE OR REPLACE TABLE `dh-darkstores-live.csm_automated_tables.flat_rb_contract_info`
AS
...
WITH contract AS (
  ...
  FROM (
      ...
      FROM `fulfillment-dwh-production.curated_data_shared_salesforce_srm.contract` AS c
      WHERE TRUE
        AND REGEXP_CONTAINS(c.global_entity_id, "HF_EG|PY_CL|FP_SG|FP_TH|NP_HU|GV_ES|TB_JO|TB_KW|TB_OM|PY_AR|PY_PE|TB_AE|TB_QA|YS_TR|GV_UA|GV_IT|TB_BH|FP_HK|FP_PH|HS_SA")
        ...
        AND (
          ...
          OR (
            c.status = 'Active'
            AND DATE_TRUNC('2025-12-01', month) BETWEEN DATE_TRUNC(c.startdate, month)
            AND DATE_TRUNC(c.enddate, month)
          )
          OR (c.status = 'Inactive'
          AND DATE_TRUNC('2025-12-01', month) <= DATE_TRUNC(c.effective_end_date__c, month))
        )
      ) AS temp
...
contractterm AS (
  SELECT ct.gsid__c AS global_supplier_id,
    ...
    MAX(
      CASE
        WHEN ct.srm_startdate__c <= DATE('2025-12-01') THEN ct.srm_startdate__c
        ELSE NULL
      END
    ) OVER (
  ...
  FROM `fulfillment-dwh-production.curated_data_shared_salesforce_srm.srm_contracttradingterm__c` AS ct
  WHERE TRUE
    AND REGEXP_CONTAINS(ct.global_entity_id, "HF_EG|PY_CL|FP_SG|FP_TH|NP_HU|GV_ES|TB_JO|TB_KW|TB_OM|PY_AR|PY_PE|TB_AE|TB_QA|YS_TR|GV_UA|GV_IT|TB_BH|FP_HK|FP_PH|HS_SA")
),
...
opp AS (
  SELECT o.id AS opp_id,
    ...
    o.srm_startdate__c,
    o.srm_enddate__c
  FROM `fulfillment-dwh-production.curated_data_shared_salesforce_srm.opportunity` AS o
  WHERE TRUE
    AND o.agreement_type__c = 'Informal (No Contract)'
    AND o.stagename = 'Accepted'
    AND REGEXP_CONTAINS(o.global_entity_id, "HF_EG|PY_CL|FP_SG|FP_TH|NP_HU|GV_ES|TB_JO|TB_KW|TB_OM|PY_AR|PY_PE|TB_AE|TB_QA|YS_TR|GV_UA|GV_IT|TB_BH|FP_HK|FP_PH|HS_SA")
  GROUP BY ALL
),
...
opp_term AS(
  SELECT ot.gsid__c AS global_supplier_id,
    ...
    MAX(
      CASE
        WHEN ot.srm_startdate__c <= DATE('2025-12-01') THEN ot.srm_startdate__c
        ELSE NULL
      END
    ) OVER (
  ...
  FROM `fulfillment-dwh-production.curated_data_shared_salesforce_srm.srm_opportunitytradingterm__c` AS ot
  WHERE TRUE
    AND REGEXP_CONTAINS(ot.global_entity_id, "HF_EG|PY_CL|FP_SG|FP_TH|NP_HU|GV_ES|TB_JO|TB_KW|TB_OM|PY_AR|PY_PE|TB_AE|TB_QA|YS_TR|GV_UA|GV_IT|TB_BH|FP_HK|FP_PH|HS_SA")
),
...
brand AS (
  SELECT b.id AS brand_id,
    ...
    /*b.brand_name__c,*/
  FROM `fulfillment-dwh-production.curated_data_shared_salesforce_srm.brand__c` AS b
  GROUP BY ALL
),
...
account AS (
  ...
  WHERE TRUE
    AND REGEXP_CONTAINS(a.global_entity_id, "HF_EG|PY_CL|FP_SG|FP_TH|NP_HU|GV_ES|TB_JO|TB_KW|TB_OM|PY_AR|PY_PE|TB_AE|TB_QA|YS_TR|GV_UA|GV_IT|TB_BH|FP_HK|FP_PH|HS_SA")
  GROUP BY ALL
),
...
user AS (
  ...
  FROM `fulfillment-dwh-production.curated_data_shared_salesforce_srm.user` AS u
  GROUP BY ALL
),
...
contracttradingtermtier AS (
  ...
  FROM `fulfillment-dwh-production.curated_data_shared_salesforce_srm.srm_contracttradingtermtier__c` AS ctt
  GROUP BY ALL
)
